...
Apple Sign In uses OpenID Connect (OIDC) with some unique requirements compared
to other providers.
...
Your Team ID is in the top-right of the Apple Developer Portal, or in
**Membership Details**. It's a 10-character alphanumeric string.

## Important Notes

### POST Callback

Apple uses `response_mode=form_post`, so the OAuth callback comes via POST
request instead of GET. Cross Auth handles this automatically.

### First-Time Data Only

Apple sends the user's name and email **only on the first authorization**. On
subsequent logins, you only get the user ID. Store user data immediately when
...
Users can choose to hide their real email address. Apple provides a private
relay address like `abc123@privaterelay.appleid.com`. Check the
`is_private_email` field:
...
Unlike other OAuth providers that use a static client secret, Apple requires a
JWT signed with your private key. Cross Auth generates this automatically on
each token exchange.
...
Your web framework isn't accepting POST requests on the callback URL. Cross
Auth's `AuthRouter` handles this automatically, but if using a custom setup,
